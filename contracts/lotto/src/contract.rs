...
pub fn execute_withdraw(
    ...
    // Get the total supply of aust in the pool
    let shares_supply = pool.lottery_shares + pool.deposit_shares + pool.sponsor_shares;

    // Get the aust owned by the contract. This should equal shares_supply aside from small rounding differences
    let contract_a_balance = query_token_balance(
        &deps.querier,
        config.a_terra_contract.clone(),
        env.clone().contract.address,
    ...
    // Validate that the depositor has a non zero supply of aust
    ...
    // Validate that there isn't a lottery running
    let current_lottery = read_lottery_info(deps.storage, state.current_lottery);
    if current_lottery.rand_round != 0 {
        return Err(ContractError::LotteryAlreadyStarted {});
    }

    // Validate that the ust value of the aust stored in the smart contract
    // is at least as the amount of ust that has been deposited into the pool.
    // This check should never fail but is implemented to minimize the damage of potential exploits
    if Decimal256::from_uint256(Uint256::from(contract_a_balance)) * rate
        < (pool.total_deposits + pool.total_sponsor_amount)
    {
    ...
    // Get the fraction of the contracts shares that belong to the depositor
    let depositor_ratio = depositor.shares / shares_supply;

    // Get the number of aust shares that belong to the depositor
    ...
    if let Some(amount) = amount {
        if Uint256::from(amount) > pooled_deposits {
            // If trying to withdraw more than the value of the aust
            // then return an InvalidWithdraw error
            return Err(ContractError::InvalidWithdraw {});
        } else {
            // Set withdraw_ratio to the requested withdraw amount divided by the ust value of the depositors aust
            withdraw_ratio = Decimal256::from_ratio(Uint256::from(amount), pooled_deposits);
        }
    }

    // Get the number of aust shares to redeem
    let aust_to_redeem = aust_amount * withdraw_ratio;

    // Get the ust value of the aust shares being redeemed
    // This should be close to "amount" but potentially different because of rounding errors
    let redemed_amount = pooled_deposits * withdraw_ratio;

    // Get the value of the returned ust amount post tax
    ...
    // Get the number of tickets the depositor has
    let tickets_amount = depositor.tickets.len() as u128;

    // Get the rounded number of tickets that are being withdrawn
    let rounded_tickets = Uint256::from(tickets_amount) * withdraw_ratio;

    // Get the decimal number of tickets that are being withdrawn
    let decimal_tickets = Decimal256::from_uint256(Uint256::from(tickets_amount)) * withdraw_ratio;

    // Set withdrawn tickets to the ceiling of decimal_tickets
    ...
    // Validate that there aren't more tickets being withdrawn than the user owns.
    // This should never happen but is in place as a double check.
    if withdrawn_tickets > tickets_amount {
        return Err(ContractError::InvalidWithdraw {});
    }

    // Drain "withdrawn_tickets" amount of the users tickets
    ...
    // Update the global state and pool
