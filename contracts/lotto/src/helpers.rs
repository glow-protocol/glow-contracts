...
pub fn compute_reward(state: &mut State, pool: &Pool, block_height: u64) {
    ...
    let total_deposited = pool.total_user_lotto_deposits + pool.total_sponsor_deposits;
...
pub fn compute_depositor_reward(state: &State, depositor: &mut DepositorInfo) {
    depositor.pending_rewards += Decimal256::from_uint256(depositor.lotto_deposit)
...
pub fn compute_sponsor_reward(state: &State, sponsor: &mut SponsorInfo) {
    sponsor.pending_rewards += Decimal256::from_uint256(sponsor.lotto_deposit)
